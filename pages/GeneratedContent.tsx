...
const GeneratedContent: React.FC = () => {
  ...
  const confirmReview = async () => {
      ...
      // Fix: Cast fallback to any to avoid TypeScript property access errors on empty object
      const content = viewingTopic.generatedContent || ({} as any);
      
      // Payload structure requested: Title, HTML, Reason, Page ID (plus status and SEO data for completeness)
      const payload = {
          title: content.title || viewingTopic.title,
          html_content: content.content_html || viewingTopic.htmlContent || '',
          reason: reason,
          page_id: viewingTopic.pageId || '', // Include page ID
          status: action,
          // Keeping extra metadata just in case user needs it later
      ...
      try {
          // Promise array for concurrent execution
          const promises = [sendArticleReview(articleReviewWebhookUrl, payload)];
          
          // If action is DRAFT, also fire the drafting webhook
          ...
          // Close viewer immediately to return to list
          setViewingTopic(null);
          setReviewStatus('idle');
          ...
          if (msg.includes('410') || msg.includes('404')) {
              msg = `Webhook Connection Failed (${msg}).\n\nPossible Causes:\n1. The Make.com scenario is inactive (OFF).\n2. The Webhook URL in Settings is incorrect.\n\nPlease check your Make.com dashboard.`;
          }
          alert(msg);
      } finally {
          setReviewModalState(prev => ({ ...prev, isOpen: false }));
  ...
  // Robust helper to extract scores from potentially messy data structures
  const getScores = (validatorData: any) => {
      if (!validatorData) return {};
      
      // 1. Try explicit paths for scores object
      ...
      if (explicitScores && typeof explicitScores === 'object' && Object.keys(explicitScores).length > 0) {
           // Validate it actually contains numbers
      ...
      // 2. Try flat keys in result or root
      const knownKeys = ['b2b_tone', 'brand_alignment', 'structure', 'accuracy', 'enterprise_relevance', 'seo_optimization', 'depth', 'relevance'];
    ...
    return (
        <div className="fixed inset-0 bg-white dark:bg-gray-900 z-50 flex flex-col animate-fadeIn transition-colors duration-200">
            ...
            <style>{`
                ...
                .article-preview h1 { font-size: 2.25rem; line-height: 2.5rem; font-weight: 700; margin-bottom: 1.5rem; margin-top: 0; color: ${theme === 'dark' ? '#f3f4f6' : '#111827'}; }
                .article-preview h2 { font-size: 1.5rem; line-height: 2rem; font-weight: 600; margin-top: 2.5rem; margin-bottom: 1rem; color: ${theme === 'dark' ? '#e5e7eb' : '#1f2937'}; border-bottom: 1px solid ${theme === 'dark' ? '#374151' : '#e5e7eb'}; padding-bottom: 0.5rem; }
                .article-preview h3 { font-size: 1.25rem; line-height: 1.75rem; font-weight: 600; margin-top: 2rem; margin-bottom: 0.75rem; color: ${theme === 'dark' ? '#d1d5db' : '#374151'}; }
                .article-preview p { margin-bottom: 1.25rem; color: ${theme === 'dark' ? '#9ca3af' : '#374151'}; font-size: 1.05rem; }
                .article-preview ul { margin-bottom: 1.25rem; padding-left: 1.5rem; list-style-type: disc; color: ${theme === 'dark' ? '#9ca3af' : '#374151'}; }
                .article-preview ol { margin-bottom: 1.25rem; padding-left: 1.5rem; list-style-type: decimal; color: ${theme === 'dark' ? '#9ca3af' : '#374151'}; }
                .article-preview li { margin-bottom: 0.5rem; }
                .article-preview strong { font-weight: 600; color: ${theme === 'dark' ? '#f3f4f6' : '#111827'}; }
                .article-preview a { color: ${theme === 'dark' ? '#818cf8' : '#4f46e5'}; text-decoration: underline; text-underline-offset: 2px; }
                .article-preview blockquote { border-left: 4px solid #6366f1; padding-left: 1rem; font-style: italic; color: ${theme === 'dark' ? '#9ca3af' : '#4b5563'}; background-color: ${theme === 'dark' ? '#1f2937' : '#f9fafb'}; py-2; my-4; }
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-3 text-sm text-gray-500 dark:text-gray-400">
                             ...
                             <span>•</span>
                             <span className="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-2 py-0.5 rounded text-xs font-medium">{viewingTopic.product}</span>
                             {avgScore && (
                                <>
                                    <span>•</span>
            ...
            <div className="flex flex-1 overflow-hidden relative">
                ...
                <div className="flex-1 flex flex-col overflow-hidden relative">
                    ...
                    <div className="border-b border-gray-200 dark:border-gray-700 px-6 bg-gray-50 dark:bg-gray-850 flex gap-6 flex-shrink-0">
                        <button
                            ...
                            className={`py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeViewTab === 'preview' ? 'border-indigo-600 text-indigo-600 dark:text-indigo-400 dark:border-indigo-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}
                        >
                            <FileText className="w-4 h-4" /> Article Preview
                        </button>
                        <button
                            onClick={() => setActiveViewTab('details')}
                            className={`py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeViewTab === 'details' ? 'border-indigo-600 text-indigo-600 dark:text-indigo-400 dark:border-indigo-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}
                        >
                        ...
                        {showValidationTab && (
                            <button
                                ...
                                className={`py-3 text-sm font-medium border-b-2 transition-colors flex items-center gap-2 ${activeViewTab === 'validation' ? 'border-indigo-600 text-indigo-600 dark:text-indigo-400 dark:border-indigo-400' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-200'}`}
                            >
                        ...
                        {activeViewTab === 'preview' && (
                            <div className="max-w-4xl mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 md:p-12 min-h-full">
                                ...
                                {generatedContent?.featured_image && (
                                    <div className="mb-8 rounded-xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-700">
                                        <img 
                                            src={generatedContent.featured_image} 
                                            alt={generatedContent.title || "Featured"} 
                        ...
                        {activeViewTab === 'validation' && (
                            <div className="max-w-5xl mx-auto space-y-6">
                                ...
                                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                                    {/* Header */}
                                    <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-700 bg-gray-50/30 dark:bg-gray-750/30 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                        <h3 className="font-bold text-gray-900 dark:text-white flex items-center gap-2 text-lg">
                                            <BarChart2 className="w-5 h-5 text-indigo-600 dark:text-indigo-400" /> 
                                            Content Quality Analysis
                                        </h3>
                                        {avgScore && (
                                            <span className={`px-4 py-1.5 rounded-full text-sm font-bold border ${getScoreColor(Number(avgScore))} self-start sm:self-auto`}>
                                        ...
                                        <div className="grid md:grid-cols-2 gap-8">
                                            <div>
                                                ...
                                                <ul className="space-y-3">
                                                    {(validatorData.result?.reasons || (validatorData as any).reasons || []).map((r: string, i: number) => (
                                                        <li key={i} className="flex items-start gap-3 text-sm text-gray-600 dark:text-gray-300 bg-green-50/50 dark:bg-green-900/10 p-3 rounded-lg border border-green-50 dark:border-green-900/30">
                                                            <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-green-500 flex-shrink-0"></span>
                                                            <span className="leading-relaxed">{r}</span>
                                                        </li>
                                                    ))}
                                                    {(!(validatorData.result?.reasons || (validatorData as any).reasons)) && (
                                                        <li className="text-gray-400 text-sm italic">No strengths listed.</li>
                                                    )}
                                                </ul>
                                            </div>
                                            <div>
                                                <h4 className="font-semibold text-gray-900 dark:text-white mb-4 flex items-center gap-2 border-b border-gray-100 dark:border-gray-700 pb-2">
                                                    <Info className="w-5 h-5 text-blue-500" /> Recommendations
                                                </h4>
                                                <ul className="space-y-3">
                                                    {(validatorData.result?.recommendations || (validatorData as any).recommendations || []).map((r: string, i: number) => (
                                                        <li key={i} className="flex items-start gap-3 text-sm text-gray-600 dark:text-gray-300 bg-blue-50/50 dark:bg-blue-900/10 p-3 rounded-lg border border-blue-50 dark:border-blue-900/30">
                                                            <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-blue-500 flex-shrink-0"></span>
                                                            <span className="leading-relaxed">{r}</span>
                                                        </li>
                                                    ))}
                                                    {(!(validatorData.result?.recommendations || (validatorData as any).recommendations)) && (
                                                        <li className="text-gray-400 text-sm italic">No recommendations listed.</li>
                                                    )}
                                                </ul>
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex flex-col items-center justify-center h-64 text-gray-400 dark:text-gray-500">
                                        ...
                                        <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">The validation step may have failed or returned incomplete data.</p>
                                    </div>
                                )}
                            </div>
                        )}
                ...
                <div className="w-64 bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700 p-6 flex flex-col gap-4 shadow-xl z-20 flex-shrink-0">
                    ...
                    {isActionable ? (
                        <>
                            <button 
                                onClick={() => initiateReview('DRAFT')}
                                disabled={reviewStatus === 'sending'}
                                className={`w-full py-3 px-4 rounded-lg font-semibold text-white flex items-center justify-center gap-2 shadow-sm transition-all transform active:scale-95 ${
                                    reviewStatus === 'sending' 
                                    ? 'bg-green-400 cursor-wait' 
                                    : 'bg-green-600 hover:bg-green-700 hover:shadow-green-200 dark:hover:shadow-none'
                                }`}
                            >
                                {reviewStatus === 'sending' && reviewModalState.action === 'DRAFT' ? (
                            ...
                            <button 
                                onClick={() => initiateReview('REJECTED')}
                                disabled={reviewStatus === 'sending'}
                                className={`w-full py-3 px-4 rounded-lg font-semibold text-white flex items-center justify-center gap-2 shadow-sm transition-all transform active:scale-95 ${
                                    reviewStatus === 'sending' && reviewModalState.action === 'REJECTED'
                            ...
                            <div className="mt-auto pt-6 border-t border-gray-100 dark:border-gray-700 text-xs text-gray-400 dark:text-gray-500 text-center">
                                Reviewing will send data to the configured webhook.
                            </div>
                        </>
                    ) : (
                        <div className={`p-4 rounded-lg border text-center ${
                            viewingTopic.status === TopicStatus.ARTICLE_DRAFT 
                            ...
                            <p className="font-bold text-sm">
                                {viewingTopic.status === TopicStatus.ARTICLE_DRAFT ? 'Article Drafted' : 'Article Rejected'}
                            </p>
                            <p className="text-xs mt-2 opacity-80">
                                This article has been finalized and can no longer be modified.
                            </p>
                        </div>
                    )}
                </div>
             ...
             <ConfirmationModal 
                 ...
                 message={`Are you sure you want to permanently delete the article for "${deleteModalState.topicTitle}"?`} 
                 variant="danger" 
                 confirmLabel="Delete" 
             />

             {/* Review Confirmation Modal (with Reason) */}
             <ConfirmationModal
                 isOpen={reviewModalState.isOpen}
                 onClose={() => setReviewModalState(prev => ({ ...prev, isOpen: false }))}
                 onConfirm={confirmReview}
                 title={reviewModalState.action === 'DRAFT' ? 'Confirm Approval' : 'Confirm Rejection'}
                 message={
                     reviewModalState.action === 'DRAFT' 
                     ? `Are you sure you want to approve "${viewingTopic.title}" as a draft?` 
                     : `Are you sure you want to reject "${viewingTopic.title}"?`
                 }
                 variant={reviewModalState.action === 'DRAFT' ? 'success' : 'danger'}
                 confirmLabel={reviewModalState.action === 'DRAFT' ? 'Approve & Send' : 'Reject & Send'}
                 showInput={true}
                 inputPlaceholder="Reason (Required)..."
                 inputValue={reviewModalState.reason}
                 onInputChange={(val) => setReviewModalState(prev => ({ ...prev, reason: val }))}
             />
        </div>
    );
  }
  ...
  const tabs = [
      { id: 'review', label: 'Ready for Review', icon: FileText, color: 'text-indigo-600 dark:text-indigo-400', count: counts.review },
      { id: 'drafts', label: 'Drafts / Approved', icon: CheckCircle, color: 'text-green-600 dark:text-green-400', count: counts.drafts },
      { id: 'rejected', label: 'Rejected', icon: XCircle, color: 'text-red-600 dark:text-red-400', count: counts.rejected },
  ];
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 flex-shrink-0">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative">
                ...
                className="pl-9 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none w-full sm:w-64 bg-white dark:bg-gray-750 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500"
                />
            </div>
            <div className="relative">
                <select 
                ...
                className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none appearance-none bg-white dark:bg-gray-750 text-gray-900 dark:text-gray-100 cursor-pointer"
                >
              ...
              return (
                  <button
                      ...
                      className={`
                          ...
                          ${isActive 
                              ? `border-${tab.color.split('-')[1].split(' ')[0]}-500 ${tab.color} bg-${tab.color.split('-')[1].split(' ')[0]}-50/50 dark:bg-${tab.color.split('-')[1].split(' ')[0]}-900/20`
      ...
      <div className="flex-1 overflow-y-auto">
        {filteredTopics.length === 0 ? (
            <div className="text-center py-20 bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 border-dashed h-full flex flex-col items-center justify-center">
                ...
                {activeTab === 'review' && (
                    <p className="text-gray-400 dark:text-gray-500 text-sm mt-1">Go to "Topic Results" &gt; "Approved" to generate content.</p>
                )}
            </div>
        ) : (
                        ...
                        <div className="p-5 flex-1">
                            <div className="flex justify-between items-start mb-3">
                                ...
                                <span className={`bg-${statusColor}-100 dark:bg-${statusColor}-900/30 text-${statusColor}-700 dark:text-${statusColor}-300 px-2 py-1 rounded-full text-xs font-semibold border border-${statusColor}-200 dark:border-${statusColor}-800 flex items-center gap-1`}>
                                    ...
                                    {topic.status === TopicStatus.ARTICLE_DRAFT ? 'Drafted' : topic.status === TopicStatus.ARTICLE_REJECTED ? 'Rejected' : 'Ready'}
                                </span>
                            </div>
                            ...
                            <div className="flex flex-col gap-2 mb-4">
                                ...
                                {avg && (
                                    <div className="flex">
                                        <span className={`flex items-center gap-1 text-xs font-bold px-2 py-1 rounded border ${getScoreColor(Number(avg))}`}>
                                            <BarChart2 className="w-3 h-3" /> AI Score: {avg}/10
                                        </span>
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="border-t border-gray-100 dark:border-gray-700 p-4 bg-gray-50/50 dark:bg-gray-750/50 rounded-b-xl flex justify-between items-center">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => openViewer(topic)} className={`px-4 py-2 bg-${statusColor}-600 hover:bg-${statusColor}-700 text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2 shadow-sm shadow-${statusColor}-200 dark:shadow-none`}>
                                    ...
                                    {activeTab === 'review' ? 'Read Article' : 'View Details'}
                                </button>
                            </div>
                        </div>
                    </div>
